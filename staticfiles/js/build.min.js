...
Waypoint.Adapter=NoFrameworkAdapter}());var Util=function(){var transition=false;var MAX_UID=1000000;var TransitionEndEvent={WebkitTransition:'webkitTransitionEnd',MozTransition:'transitionend',OTransition:'oTransitionEnd otransitionend',transition:'transitionend'};function toType(obj){return{}.toString.call(obj).match(/\s([a-zA-Z]+)/)[1].toLowerCase();}
...
var $trigger=$(this);var selector=Util.getSelectorFromElement(this);$(selector).each(function(){var $target=$(this);var data=$target.data(DATA_KEY);var config=data?'toggle':$trigger.data();Collapse._jQueryInterface.call($target,config);});});$.fn[NAME]=Collapse._jQueryInterface;$.fn[NAME].Constructor=Collapse;$.fn[NAME].noConflict=function(){$.fn[NAME]=JQUERY_NO_CONFLICT;return Collapse._jQueryInterface;};return Collapse;}($);$(document).ready(function(){$('body').delay(300).css('opacity','1');$('ul.nav li.dropdown').hover(function(){$(this).find('.dropdown-menu').stop(true,true).delay(100).fadeIn(300);},function(){$(this).find('.dropdown-menu').stop(true,true).delay(100).fadeOut(300);});if($('body.index').length){var waypoint=new Waypoint({element:document.getElementById('summary'),handler:function(direction){$('.triggers .figure').each(function(){$(this).prop('Counter',0).animate({Counter:$(this).text()},{duration:4000,easing:'swing',step:function(now){$(this).text(Math.ceil(now));}});});waypoint.destroy()}});}
if($('body div').hasClass('display')){$(window).scroll(function(){$('.display').each(function(){var imagePos=$(this).offset().top;var topOfWindow=$(window).scrollTop();if(imagePos<topOfWindow+700){$(this).addClass("show");};});});}
switch(new Date().getMonth()){case 0:$('#summary .triggers .item:first-child .figure').text('117');$('#summary .triggers .item:nth-child(2) .figure').text('89');break;case 1:$('#summary .triggers .item:first-child .figure').text('118');$('#summary .triggers .item:nth-child(2) .figure').text('90');break;case 2:$('#summary .triggers .item:first-child .figure').text('119');$('#summary .triggers .item:nth-child(2) .figure').text('91');break;case 3:$('#summary .triggers .item:first-child .figure').text('120');$('#summary .triggers .item:nth-child(2) .figure').text('92');break;case 4:$('#summary .triggers .item:first-child .figure').text('121');$('#summary .triggers .item:nth-child(2) .figure').text('91');break;case 5:$('#summary .triggers .item:first-child .figure').text('122');$('#summary .triggers .item:nth-child(2) .figure').text('92');break;case 6:$('#summary .triggers .item:first-child .figure').text('123');$('#summary .triggers .item:nth-child(2) .figure').text('93');break;case 7:$('#summary .triggers .item:first-child .figure').text('124');$('#summary .triggers .item:nth-child(2) .figure').text('94');break;case 8:$('#summary .triggers .item:first-child .figure').text('125');$('#summary .triggers .item:nth-child(2) .figure').text('95');break;case 9:$('#summary .triggers .item:first-child .figure').text('126');$('#summary .triggers .item:nth-child(2) .figure').text('96');break;case 10:$('#summary .triggers .item:first-child .figure').text('127');$('#summary .triggers .item:nth-child(2) .figure').text('97');break;case 11:$('#summary .triggers .item:first-child .figure').text('128');$('#summary .triggers .item:nth-child(2) .figure').text('98');}
var form=$('#contacts');$(form).submit(function(e){e.preventDefault();var formData=$(form).serialize();$.ajax({type:'POST',url:$(form).attr('action'),data:formData}).done(function(response){$('#contacts .btn').text('Message has been sent').addClass('sent').attr('disabled',true);$('#name').val('');$('#email').val('');$('#message').val('');}).fail(function(data){if(data.responseText!==''){console.log(data.responseText);}else{console.log('Oops! An error occured and your message could not be sent.');}});});});